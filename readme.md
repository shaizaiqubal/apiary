<p align="center">
  <img src="assets/logo.png" alt="Apiary logo" width="360">
</p>
...
4. **Level up.** Confirmed actions add points, plants and nesting actions each carrying their own value. Enough points moves your plot up a stage: Seedling, Garden, Habitat, Sanctuary. Each stage is a genuinely better patch of ground than the last, not just a bigger number.
5. **Fill in the Bee-dex.** Somewhere along the way, real bees start showing up, because you've built somewhere for them to be. Get a decent photo of one and the app has a go at naming it, then asks you to confirm. Get it right and that species is yours for good, marked with a real photograph rather than an illustration. Rarer species score a lot more than common ones, and spotting one you've already logged on the same plot still earns something, just less, since the game would rather send you off to plant for a different bee than have you farm the same sighting twice. Once a species is confirmed, it's yours for good.
6. **Better with other people around** Your plot works fine on its own. It also sits on a shared map alongside everyone else's, with a running count of what the whole community has built between them: bare soil left, plants gone in, species confirmed, all added up across everyone playing. Nobody needs to look at that map for the game to make sense. It's there because a scattered garden here and a balcony box there add up to something worth seeing, and it's nice to know your patch isn't the only one. (change)
...
- **Frontend:** React (Vite, JavaScript), React Router DOM v7, Axios, react-leaflet with CartoDB Positron tiles.
- **Backend:** FastAPI, PostgreSQL via SQLAlchemy.
- **Deployment:** Postgres on Render, API as a Dockerized service on Google Cloud Run, frontend on Vercel.
- **Identity:** No auth. Client generates a UUID with `crypto.randomUUID()` on first load, persisted in `localStorage`, attached as an `X-User-ID` header via an Axios request interceptor. Authenticated endpoints resolve the user through a `get_current_user_id` dependency.
- **Vision:** Two-tier LLM vision pipeline. Tier one verifies that a submitted photo plausibly matches the claimed planting or nesting action. Tier two handles species ID on sighting photos, returning the top 2 to 3 candidates for user confirmation rather than a single automated call.
- **Data:** `plants` (RHS Plants for Pollinators IDs and hardiness ratings, ~53 rows), `species` (30 UK wild bee species and rarity tiers), `nesting_actions`, and a `plant_species` junction table. Region bucketing uses latitude bands mapped to USDA hardiness zone equivalents, no geocoding API.
